...
mm="$((COLUMNS/2-26))"; 
for ii in {1..111}; do \
ll=$((mm-RANDOM%2+RANDOM%2)); for ik in {0..7}; do 
printf "\e[${ll}G${h[ik]}\e[B"; done; 
printf "\e[38;5;${ii}m\e[B"; 
for i in {0..7}; do printf "\e[${mm}G${h[i]}\e[B"; done
printf "\n\e[8A"; read -n1 -t.1 -res x; 
[ "$x" ]&&break; done; }; 
alias=kkaa='for i in {111..180}; do printf "\e[38;5;${i}m\e[B"; 
for i in {0..7}; do printf "\e[${mm}G${h[i]}\e[B"; done
printf "\e[8A"; read -n1 -t.1 -res x; 
printf "\e[$((RANDOM%6))A\e[$((RANDOM%4))B"
[ "$x" ]&&break; done; 

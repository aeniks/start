...
cd $HOME/logs/apts &>/dev/null; aapp=($(ls|fzf -i -m --color preview-bg:0 --bind "q:abort" --preview "cat {}|tail -c+1|bat -ppflzig" --preview-window "wrap,${bw},noborder" --border none --cycle --ansi --inline-info))||(printf %b "\n\n"&& cd $pwd &>/dev/null&& return 1;)||return 1; cd $pwd &>/dev/null&& 
printf %b "\n\n"; (printf %b " -- Install: "; 
printf %b "${aapp[*]}"|tr -s "\n" ","; bb="\b"; 
[[ ${#aapp[*]} == 1 ]]&& unset bb; 
